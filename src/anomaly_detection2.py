...
def visualize(img, gt, score_map, max_score, save_path=None, show_plot=True):
    ...
    peak_score_current_image = score_map.max().item() if torch.is_tensor(score_map) else np.max(score_map)
    plt.subplot(1, 3, 3)
    plt.imshow(score_map, cmap='jet')
    plt.imshow(img, cmap='gray', interpolation='none')
    plt.imshow(score_map, cmap='jet', alpha=0.5, interpolation='none')
    plt.colorbar(extend='both')
    plt.title(f'Anomaly Map (Peak Score: {peak_score_current_image:.2f})')
    plt.clim(0, max_score)

    if save_path:
        # Deconstruct the original save_path to insert the peak score
        dir_name = os.path.dirname(save_path)
        base_name = os.path.basename(save_path)
        name, ext = os.path.splitext(base_name)

        # Format the peak score for filename to be sortable
        # e.g., 25.78 -> "0025_78", 123.45 -> "0123_45", 5.67 -> "0005_67"
        score_str = f"{peak_score_current_image:07.2f}".replace('.', '_')

        modified_filename = f"{score_str}_{name}{ext}"

        # Reconstruct the full path
        if dir_name: # If there was a directory part
            actual_save_path = os.path.join(dir_name, modified_filename)
        else: # If save_path was just a filename
            actual_save_path = modified_filename
